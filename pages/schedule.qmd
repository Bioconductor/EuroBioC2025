...
fd$author <- ifelse(
    (. <- fd$presenter) != "", 
    ., sapply(ss, .subset, 1))
fd$coauth <- sapply(ss, paste, collapse=", ")
...
# add linebreak for workshops locations
as <- df[df$type == "Workshop", "author"]
as <- gsub(":\\s", ":<br>", as)
df[df$type == "Workshop", "author"] <- as
...
# render table
kbl(df[, cols_to_show], escape=FALSE, row.names=FALSE) |>
    # styling
    ...
    row_spec(
        ...
        background="lavender") |>
    row_spec(
        which(df$type %in% c("Break", "Lunch", "Group photo")), 
